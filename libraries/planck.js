 ...
 * @param {TimeStep} step
 */
Solver.prototype.solveWorld = function(step, bodies) {
    var world = this.m_world;
    // Clear all the island flags.
    // for (var b = world.m_bodyList; b; b = b.m_next) {
    for(var i = 0; i < bodies.length; i++) {
        bodies[i].m_islandFlag = false;
    }
 ...
 * @param {TimeStep} step
 */
Solver.prototype.solveWorldTOI = function(step, inputBodies) {
    var world = this.m_world;
    if (world.m_stepComplete) {
        // for (var b = world.m_bodyList; b; b = b.m_next) {
        for(var i = 0; i < inputBodies.length; i++) {
            var b = inputBodies[i];
            b.m_islandFlag = false;
            b.m_sweep.alpha0 = 0;
        }
 ...
 */
World.prototype.clearForces = function(bodies) {
...
World.prototype.stepBodies = function(timeStep, bodies, velocityIterations, positionIterations)
{
    if ((velocityIterations | 0) !== velocityIterations) {
        // TODO: remove this in future
        velocityIterations = 0;
    ...
    // Integrate velocities, solve velocity constraints, and integrate positions.
    if (this.m_stepComplete && timeStep > 0) {
        this.m_solver.solveWorld(s_step, bodies);
        // Synchronize fixtures, check for out of range bodies.
        for (var i = 0; i < bodies.length; i++) {
            // If a body was not in an island then it did not move.
            var b = bodies[i];
    ...
    // Handle TOI events.
    if (this.m_continuousPhysics && timeStep > 0) {
        this.m_solver.solveWorldTOI(s_step, bodies);
    }
    if (this.m_clearForces) {
        this.clearForces(bodies);
    }
    this.m_locked = false;
};
